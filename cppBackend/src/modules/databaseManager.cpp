...
public:
    ...
    std::string getStockData(std::string stockName, int limit){
        ...
        try {
            ...
            writeError("SQL get Data " + (std::string)e.what());
...
private:
        ...
        try {
            ...
            while (retryCount < maxRetries) {
                try {
                    ...
                    writeError("Failed to connect to the database:" + (std::string)e.what());
                }
            ...
            if (!con) {
                ...
                writeError("Failed to connect to the database after maximum retry attempts.");
            }
            ...
            writeError("SQL create table error " + (std::string)e.what());
    ...
    void insertData(std::map<std::string, std::string> stockMetadataMap, std::map<std::string, double> stockDataMap){
        ...
        try {
            ...
            writeError("SQL command error " + (std::string)e.what());
            std::cout << "Error: " << e.what() << std::endl;
        }
    }
    ...
    std::string readMetaStockValuesFromJson(nlohmann::json j, std::string key1, std::string key2) {
        ...
        try {
            ...
            writeError("Stocks read from json error " + key1 + " " + key2);
        }
        return value;
    };
    ...
    double readStockValuesFromJson(nlohmann::json j, std::string key1, std::string key2, std::string key3) {
        ...
        try {
            ...
            writeError("Stocks read from json error " + key1 + " " + key2 + " " + key3);
        }
        return value;
    };
};